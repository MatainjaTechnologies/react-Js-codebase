            ...
            <div className="tab-content row">
                ...
                <div className="block bg-grey">
                    <div className="player-stats-main block bg-white shadow radius-1">
                        ...
                        <div className="box border-bottom border-left">{/*Clean Sheets*/}
                        { Cookies.get('googtrans') && Cookies.get('googtrans').length > 0 && Cookies.get('googtrans').split("/").pop() =='my' ?

                            <span class="notranslate">ပစ်မှတ်ပေါ် ကန်သည်</span> 
                            : <span>Shoots Of Target</span> 
                        }
                        ...
                        <div className="box border-left border-top"><strong>{playerInfo.pass_accurate===''? 0 : playerInfo.pass_accurate}</strong> <br />{/*Pass Accuracy*/}
                        { Cookies.get('googtrans') && Cookies.get('googtrans').length > 0 && Cookies.get('googtrans').split("/").pop() =='my' ?

                            <span class="notranslate">ကူညီထောက်ပံ့ပေးမှု</span> 
                            : <span>Assits</span> 
                        }
                        </div>
                    </div>
                        ...
                        <ul>
                            <li>
                                { Cookies.get('googtrans') && Cookies.get('googtrans').length > 0 && Cookies.get('googtrans').split("/").pop() =='my' ?
                                    ...
                                    <span class="notranslate">ဂိုး</span> 
                                    : <span>Goals</span> 
                                }

                                
                                <span class="notranslate">{playerInfo.goals===''? 0 : playerInfo.goals}</span>
                            </li>

                            <li>
                                <span>Penalties Scores</span>
                                <span class="notranslate">{playerInfo.penalty_scores===''? 0 : playerInfo.penalty_scores}</span>
                            </li>
                            <li>
                                <span>Duels Won</span>
                                <span class="notranslate">{playerInfo.duel_own===''? 0 : playerInfo.duel_own}</span>
                            </li>
                            <li>
                                <span>Total Shots</span>
                                <span class="notranslate">{playerInfo.short_total===''? 0 : playerInfo.short_total}</span>
                            </li>
                            <li>
                                <span>Shots on Target</span>
                                <span class="notranslate">{playerInfo.short_on_target===''? 0 : playerInfo.short_on_target}</span>
                            </li>
                            <li>
                                <span>Shots off target</span>
                                <span class="notranslate">{playerInfo.short_off_target===''? 0 : playerInfo.short_off_target}</span>
                            </li>
                            <li>
                                <span>Dribbles</span>
                                <span class="notranslate">{playerInfo.dribbles_attempts===''? 0 : playerInfo.dribbles_attempts}</span>
                            </li>
